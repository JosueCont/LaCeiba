        ...
        <LayoutV4 overlay={true}>
            ...
            {/* {groupFounded && <Text fontFamily={'titleConfortaaRegular'} color={Colors.primary} textAlign={'center'} fontSize={'lg'} mb={8}> {groupFounded?.area ? groupFounded?.area?.name : ''} </Text>} */}
            {
                groupFounded && group && groupFounded?.schedules?.sort((a, b) => {
                    ...
                    return(
                        value.isActive && (
                            <TouchableOpacity key={index} onPress={() => {navigation.navigate('FixedGroupDetail', {schedule: value, groupData: groupFounded, userId: appDuck.user.id, minPeople: minPeople != null ? minPeople : value?.area ? value?.area?.minPeople : 1, maxPeople: maxPeople != null ? maxPeople : value?.area ? value?.area?.maxPeople : 1, blocked: !exist, areaGroup: value.area});}}>
