    ...
    const confirmBooking = async () => {
        try {
            ...
            const body = {
                ...
                areaId : groupData?.area ? groupData.area.id : groupData?.schedules[0]?.area?.id,
    ...
    return(
        ...
        <LayoutV4 overlay={true}>
            <View flex={1} mx={4}>
                ...
                <Text fontFamily={'titleConfortaaRegular'} color={Colors.primary} textAlign={'center'} fontSize={'lg'}>{groupData?.area ? groupData.area.service.name : groupData?.schedules[0]?.area?.service?.name} | {groupData?.area ? groupData.area.name: groupData?.schedules[0]?.area?.name}</Text>
                ...
                text={`¿Estás seguro que deseas agendar el grupo fijo?\n\n ${groupData.name} \n ${groupData?.area ? groupData.area.service.name : groupData?.schedules[0]?.area?.service?.name} (${groupData?.area ? groupData.area.name : groupData?.schedules[0]?.area?.name}) \n${getNextDayOfWeek(dayWeek[schedule.day].id)} \n\n Miembros:\n ${membersFormated()}`}
                title={'Confirmación'}
                textButton={'Sī'}
                textNoButton={'No'}
                iconType={'check'}
